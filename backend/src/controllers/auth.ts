...
export const login = async (req: Request, res: Response) => {
  ...
  try {
    // 1. Verify that 'message' is "Authenticate with Attestra Protocol"
    if (message !== "Authenticate with Attestra Protocol") {
    ...
    // 4. Issue JWT
    const jwtSecret = process.env.JWT_SECRET || 'attestra_fallback_secret_67890';
...
export const verifyDID = async (req: any, res: Response) => {
  ...
  try {
    ...
    console.log(`[Attestra DID] Verifying identity for ${address} with DID: ${did}`);
