...
export const submitProof = async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    try {
      ...
      console.log(`[Attestra] DNAProof Oracle Verification Success for Hash: ${proof.proofData}`);
    } catch (dnaError: any) {
      console.error('[Attestra] DNAProof Verification Error:', dnaError);
    ...
    try {
      ...
      if (adminSecret) {
        ...
        console.log(`[Attestra] Soroban Execution Successful. Hash: ${sendResponse.hash}`);
      } else {
        console.warn('[Attestra] SOROBAN_ADMIN_SECRET not set, falling back to mock txHash.');
        proof.txHash = '0x' + Math.random().toString(16).padEnd(64, '0').substring(2, 66);
      }
    } catch (stellarError: any) {
      console.error('[Attestra] Error submitting to Soroban:', stellarError);
