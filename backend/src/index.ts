...
const MONGODB_URI = process.env.MONGODB_URI || 'mongodb://localhost:27017/attestra';
...
io.on('connection', (socket) => {
  console.log('[Attestra] Client connected:', socket.id);
  
  socket.on('join_protocol', (walletAddress) => {
    socket.join(walletAddress);
    console.log(`[Attestra] Admin/User ${walletAddress} joined telemetry stream.`);
  });

  socket.on('disconnect', () => {
    console.log('[Attestra] Client disconnected');
  });
});
...
try {
  ...
  try {
    ...
    console.error('[Attestra] Failed to load openapi.yaml from dist or src');
...
app.get('/health', (req: any, res: any) => {
  res.json({ status: 'Attestra Backend Online', realtime: 'Active' });
});

httpServer.listen(PORT, () => {
  console.log(`Attestra server running on port ${PORT} (HTTP + WebSockets)`);
});
