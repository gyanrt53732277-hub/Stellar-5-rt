...
export const initAutomationWorker = (io: any) => {
  console.log('[Attestra] Initializing Automation Worker...');

  // Run every minute
  cron.schedule('* * * * *', async () => {
    console.log('[Attestra] Running Automation Health Check...');
    
    try {
      // 0. Check Protocol Halt Status
      const settings = await SystemSetting.findOne();
      if (settings?.protocolHalt) {
        console.log('[Attestra] Protocol is HALTED. Skipping automation cycle.');
        return;
      }
      ...
      for (const op of pendingOps) {
        ...
        if (rule && rule.automationConfig?.autoExecute) {
          console.log(`[Attestra] Auto-executing Proof: ${op._id}`);
          ...
          if (rule.useGasAbstraction) {
            ...
            if (!user || user.gasBalance < estimatedGas) {
              console.log(`[Attestra] Insufficient gas balance for ${rule.creator}`);
            ...
            await user.save();
            console.log(`[Attestra] Deducted ${estimatedGas} gas credits from ${rule.creator}`);
          }
          ...
          if (isCrossChain) {
            console.log(`[Attestra] CROSS-CHAIN DISPATCH: Sending payload to ${rule.targetChain}`);
          }
        }
      }
      ...
      for (const rule of scheduledRules) {
        console.log(`[Attestra] Triggering Scheduled Rule: ${rule.name}`);
        
        // PHASE 2: Verifiable External Data Fetching
        let dataVerified = true;
        if (rule.dataSourceUrl) {
          try {
            console.log(`[Attestra] Fetching external data from: ${rule.dataSourceUrl}`);
            ...
            if (rule.dataSourcePath) {
              ...
              console.log(`[Attestra] Data Oracle Extracted Value:`, extracted[0]);
              // In production, cryptographically sign this extracted value here
            }

            console.log(`[Attestra] External data verified for ${rule.name}`);
          } catch (dataErr) {
            console.error(`[Attestra] Data fetch failed for ${rule.name}:`, dataErr);
            dataVerified = false;
            io.to(rule.creator).emit('execution_update', {
              type: 'ERROR',
        ...
        try {
           ...
           console.log(`[Attestra] Generating ZK Proof for ${rule.name}...`);
           ...
           console.log(`[Attestra] ZK Proof generated successfully.`);
        } catch (zkErr) {
           console.error(`[Attestra] ZK Proof generation failed:`, zkErr);
           io.to(rule.creator).emit('execution_update', {
              type: 'ERROR',
      ...
      if (verifiedProofs.length >= 3) { // Minimum batch size for aggregation
        console.log(`[Attestra] Initializing Batch Aggregation for ${verifiedProofs.length} proofs...`);
        ...
        console.log(`[Attestra] Batch Aggregation Successful: ${batch.txHash}`);
        io.emit('system_stats', {
          timestamp: new Date(),
      ...
      console.error('[Attestra] Automation Worker Error:', err);
    }
  });
};
